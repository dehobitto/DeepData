...
using DeepData.Core.Utils.Jpeg;
...
public class DctJpeg : StegoMethod<Stream, byte[]>
{
        ...
        byte[] data)
    {
        ...
        int maximumCapacity = GetCapacity(source);
        ...
        BitWorker bitWorker)
    {
        ...
        {
            for (int i = 1; i < Constants.JpegBlocksCount; i++) // Iterate through AC coefficients (skip DC at index 0)
            {
                short coefficient = currentBlock[i];

                if (IsCoefficientSkippableForEmbedding(coefficient))
                {
                    continue;
                }
                ...
                currentBlock[i] = QimHelper.QimEmbedBit(coefficient, bitWorker.ReadBit(), Options.QimDelta);
            }
    ...
    public override byte[] Extract(Stream source)
    {
        var decompressionResult = JpegHelper.Decompress(source);
        var coefficientArrays = decompressionResult.CoefArrays;
        int totalBitsToExtract = GetCapacity(source) * 3; // Assuming this factor is part of the original logic
    ...
    public override int GetCapacity(Stream source)
    {
        var decompressionResult = JpegHelper.Decompress(source);
        int capacityCount = 0;
        ...
        BitWorker bitWorker)
    {
        ProcessBlocks(blocks, componentInfo, (currentBlock) =>
        {
            for (int i = 1; i < Constants.JpegBlocksCount; i++) // Iterate through AC coefficients (skip DC at index 0)
            {
                ...
                bool extractedBit = QimHelper.QimExtractBit(coefficient, Options.QimDelta);
    ...
    private bool IsCoefficientSkippableForEmbedding(short coefficient)
    {
        return coefficient == 0 || Math.Abs(coefficient) < 2;
    }
    ...
    {
        return Math.Abs(coefficient) < 2 || coefficient == 0;
    }
